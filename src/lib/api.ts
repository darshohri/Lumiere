    ...
    if (!res.ok) {
      ...
      try {
        ...
        try {
          ...
          if (text && text.length < 120 && !text.includes('<html') && !text.includes('<!DOCTYPE')) {
            message = text;
          }
        }
      } catch {}
...
// ── Client-side Resilient Patient Store (persists across reloads even if offline) ────
const LOCAL_STORAGE_KEY = 'lumiere_local_patients';
...
function saveLocalPatient(p: Patient): void {
  ...
  try {
    ...
    const filtered = current.filter(item => item.id !== p.id && item.fhir_id !== p.fhir_id);
    filtered.unshift(p);
...
export async function fetchPatients(query?: string, searchType?: 'name' | 'phone' | 'gov'): Promise<Patient[]> {
  ...
  let serverPts: Patient[] = [];
  try {
    serverPts = await api<Patient[]>(`/api/patients?${params}`);
  } catch (e) {
    console.warn('Unable to fetch patients from API, using cached data:', e);
  }

  // Merge with locally stored patients
  const localPts = getLocalPatients();
  const mergedMap = new Map<string, Patient>();

  // Add locally created patients first
  for (const lp of localPts) {
    mergedMap.set(lp.id, lp);
  }

  // Add server patients
  for (const sp of serverPts) {
    mergedMap.set(sp.id, sp);
  }

  const allPts = Array.from(mergedMap.values());

  // If query is present and we were offline or local-only, filter here as well
  let filtered = allPts;
  if (query) {
    const qLower = query.toLowerCase();
    filtered = allPts.filter(p => {
      const fullName = `${p.given_name ?? ''} ${p.family_name ?? ''}`.toLowerCase();
      const fhir = (p.fhir_id ?? '').toLowerCase();
      const phone = p.phone ?? '';
      return fullName.includes(qLower) || fhir.includes(qLower) || phone.includes(qLower);
    });
  }
  ...
  return filtered.map(p => ({
    ...
    completeness: p.completeness ?? 90,
    tags: p.tags ?? [],
    sources: p.sources ?? ['EHR']
  }));
}
...
export async function createPatient(data: PatientCreatePayload): Promise<Patient> {
  ...
  try {
    createdPatient = await api<Patient>('/api/patients', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    });
  } catch (err: any) {
    // If server returned 400 with duplicate ID, let that bubble up so user knows
    if (err?.message && err.message.includes('already exists')) {
      throw err;
    }
    console.warn('API error during createPatient, saving to resilient local store:', err);
  }

  if (!createdPatient) {
    const fhirId = data.gov_id?.trim() || `EHR-${Math.floor(1000 + Math.random() * 9000)}`;
    createdPatient = {
      id: crypto.randomUUID ? crypto.randomUUID() : `local-${Date.now()}`,
      fhir_id: fhirId,
      given_name: data.given_name.trim(),
      family_name: data.family_name.trim(),
      name: `${data.given_name.trim()} ${data.family_name.trim()}`,
      dob: data.dob,
      gender: data.gender,
      ...
      completeness: 95,
      tags: ['New Patient'],
      sources: [data.source_system || 'Manual Entry'],
    };
  }

  // Save to client-side localStorage to guarantee zero loss
